...
YLT_REFL(UUIDPair, first, second);
...
static inline bool IsSequenceOlder(uint64_t a, uint64_t b) {
    return static_cast<int64_t>(a - b) < 0;
}

static inline bool IsSequenceEqual(uint64_t a, uint64_t b) {
    return a == b;
}

static inline bool IsSequenceNewerOrEqual(uint64_t a, uint64_t b) {
