...
struct MetadataPayload {
    ...
    uint64_t size{0};
    std::vector<ReplicaDescriptorWire> replicas;
    ...
    StandbyObjectMetadata ToStandbyMetadata(uint64_t sequence_id) const {
        ...
        for (const auto& r : replicas) {
            meta.replicas.push_back(FromWireReplicaDescriptor(r));
        }
...
class MetadataStore {
   public:
     ...
     */
    virtual const StandbyObjectMetadata* GetMetadata(const std::string& key) const = 0;
