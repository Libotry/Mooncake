                ...
                for (const auto& info : result.value()) {
                    ...
                    for (const auto& replica_desc : info.replicas) {
                        ...
                        if (replica_desc.is_memory_replica()) {
                            ...
                            replica_json["buffer_addr"] = static_cast<Json::UInt64>(mem_desc.buffer_descriptor.addr);
                            replica_json["buffer_size"] = static_cast<Json::UInt64>(mem_desc.buffer_descriptor.size);
